...
async def check_alerts() -> list[dict]:
        ...
        alerts.append(
            {
                ...
                "severity": "high",
                "message": f"Agent '{row['agent']}' has "
                f"{row['err_count']} errors in the last hour",
            }
        )

    # 2. Queue backlog
    cursor = await db.execute("SELECT COUNT(*) as cnt FROM tasks WHERE status='queued'")
    ...
    if (
        row
        and row["total"]
        and row["total"] >= ALERT_RULES["budget_daily_exceeded"]["threshold_usd"]
    ):
        alerts.append(
            {
                "type": "budget_exceeded",
                "message": f"Daily budget exceeded: ${row['total']:.2f} / "
                f"${ALERT_RULES['budget_daily_exceeded']['threshold_usd']:.2f}",
            }
        )
