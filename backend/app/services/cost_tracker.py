...
async def record_cost(
    agent: str, model: str, tokens_in: int, tokens_out: int,
    cost_usd: float = 0.0, task_id: str = None,
) -> dict:
    """Record cost entry."""
    db = await get_db()
    await db.execute(
        "INSERT INTO cost (agent, model, tokens_in, tokens_out, cost_usd, task_id) VALUES (?, ?, ?, ?, ?, ?)",
...
async def get_cost_summary(period: str = "today") -> dict:
    ...
    if period == "today":
        cursor = await db.execute(
            "SELECT agent, model, SUM(tokens_in) as total_in, SUM(tokens_out) as total_out, SUM(cost_usd) as total_cost "
