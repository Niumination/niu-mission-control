...
async def update_task(task_id: str, status: str = None, title: str = None) -> dict:
    ...
    now = datetime.now().isoformat()
    if status:
        await db.execute("UPDATE tasks SET status=?, updated_at=? WHERE id=?", (status, now, task_id))
    if title:
        await db.execute("UPDATE tasks SET title=?, updated_at=? WHERE id=?", (title, now, task_id))
    await db.commit()
