/**
 * POST /api/mc/approvals/:id/reject  { reason? }
 */
...
export async function POST(
  ...
  try {
    await decideApproval(id, 'rejected', parsed.data.reason ?? null, auth.actor.name, auth.actor.type as 'user' | 'api_key')
    return NextResponse.json({ success: true, id, decision: 'rejected' })
  } catch (err: any) {
